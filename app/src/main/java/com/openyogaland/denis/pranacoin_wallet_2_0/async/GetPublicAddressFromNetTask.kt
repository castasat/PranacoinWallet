...
class
GetPublicAddressFromNetTask(context : Context,
                            idOfUser : String)
  : Listener<String>,
    ErrorListener,
    RequestFinishedListener<StringRequest>
{
  ...
  init
  {
    if(idOfUser.isNotEmpty())
    {
      ...
      val publicAddressRequest = StringRequest(Method.GET,
                                               GET_ADDRESS_API + idOfUser,
                                               this, this)
      
      if(requestQueue != null)
      {
        publicAddressRequest.retryPolicy = DefaultRetryPolicy(DEFAULT_TIMEOUT_MS,
                                                              DEFAULT_MAX_RETRIES,
